...
插件的仓库与安装状态、历史踩坑复盘）由使用方存放在自己的**记忆区**，本 skill
只保留指针——事实变更只改记忆，skill 永不漂移，同一份 skill 可在多台机器复用。

## 0. 动手前必读（按记忆区中文名称检索，不写具体路径）

> 以下指针只给**档案中文名称**，不给文件路径：路径由使用方自己的记忆区索引维护，
> 按名称检索即可找到对应档案（本机用法：在 `$DSH_HOME/memory/` 按名称查找，
> 或直接用会话搜索工具按名称检索）。
...
4. **主环境装包**：容器测试通过后装进服务器**主环境 dsh**（保持与生产插件环境一致），
   确认：**与其它插件无冲突、渲染没问题**。
5. **桥接人工核验**：经 SSH 隧道桥接，让**用户在本机 GUI 最终人工核验**。**核验交接必须**：① 先确认隧道已就绪（本地隧道端口在监听，且**核对 ssh 进程命令行确认实际转发目标**——脚本默认映射可能被手动覆盖）；② 把**当前实际可访问的地址（含端口号）**明确告诉用户——地址/端口是机器特定事实，从记忆区「隔离测试环境档案」的拓扑表读取，**不得漏报、不得凭印象报**；③ 说明本次核验要点（看什么、怎么算过）。
...
多实例 / 多隧道并存时的拓扑纪律（**机器特定事实**——每台机器当前的容器清单、端口映射、隧道实际指向——进记忆区「隔离测试环境档案」拓扑表；本 skill 只给规则，不写死任何端口）。

**容器标注（必做）**
- 每个实验容器命名 = `dsh-web-<端口>`，并**标注当前开发内容**与状态（如 `dsh-web-3081（开发 dsh-xxx）`）；
- **空闲时把容器改为空闲状态命名**（如 `dsh-web-3081（空闲，可复用）`）——「哪个容器空着、谁在被用」必须一眼可见，用时先看标注再复用；
- **至少保留一个空闲容器**，便于随时快速进入插件开发。

**端口拓扑（每台机器一张表，三列）**
- 容器在 ECS 内的监听端口（如 3081/3082…）；
- 转 ECS 主环境（systemd 3080）的端口——仅桥接主环境时使用；
- 转主机的隧道端口（主机侧 130xx，**避开本机 dsh 的 3080** 与已占用端口）。
- ⚠️ **隧道映射可能被手动覆盖**（脚本默认 13080→3080，实际运行可能指向 3081）：使用/告知用户前，必须用 `netstat` 看本地端口 + 查 ssh 进程命令行确认**当前实际转发目标**，再报入口。

**使用前检查**
- `docker ps` 看容器状态与空闲标注；不空闲的先确认占用方再复用。
- 核对容器内插件版本与预期一致（curl 对应端口 `/plugins/<pkg>/client.js` grep 特征串）——版本不对先同步再测。

**验收后注销与还原**
- 正式验收通过、插件装回生产后，**注销该实验容器**（`docker rm -f <name>`；清理独立数据卷按档案）；
- 注销前先确认没有其它开发任务正在使用该容器；
- **永远保留至少一个空闲容器**，供下一轮插件开发直接复用；
- **保留的那个容器必须还原为「纯净官方」基线**：验收通过后把它从「插件验证环境」还原为纯净官方（卸载自定义插件、profile 重置为官方模板，重建命令按档案），并改名 `-idle`——下一轮开发从干净基线开始、按需再装插件，**不带上一轮的插件状态**；空闲期保持 `-idle` 纯净官方，谁要用谁再标注 `-dev-<插件>` 并装插件。

## 6. 维护：插件出问题怎么办

**一律按流水线处理，不直接在生产环境改/试：**
隔离环境 docker 容器测试 → 主环境装包测试 → 桥接测试 → 用户确认 → 再装回正式包。
（桥接交接按第 2 节第 5 步：确认隧道实际指向 + 告知访问地址/端口 + 核验要点；容器生命周期按第 5 节。）
