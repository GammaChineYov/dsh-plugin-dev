...
**快照对照测试（结构断言版，2026-08-17 补强）**

> 教训：曾出现「展开 C-子11-孙1 后，C-子11-孙1-曾孙1 出现在 **C-子4** 的子层级下」未被测试发现——
> 旧快照法只打印"扁平行列表 + padding 推断深度"供人眼对比：内容**存在**了、深度数字**对**了，
> 但**父节点错了**——扁平打印看不出父级归属。规则：

1. **结构快照，不是文本快照**：每条记录输出**祖先链**（如 `C > C-子11 > C-子11-孙1 > C-子11-孙1-曾孙1`），
   从 DOM 真实嵌套 / `aria-level` / 分组结构提取；**不**用 padding/缩进推断深度（启发式，不可断言）。
2. **断言祖先链（核心）**：对每个可见节点断言其祖先链 == 预期链——父级错位（挂到兄弟节点下）时
   祖先链立即不符，这正是文本存在性 / 深度数字断言抓不到的。
3. **边集快照 + 边集 diff**：把树投影为 `(parent → child)` 边集合；操作前后 diff——
   展开只允许**新增预期的边**；**任何既有边的父端改变 = 立即失败**（跨父迁移即 bug）。
4. **展开/收起往返的结构一致性 + 节点身份**：用稳定标识（`data-*` / aria-label / 节点 key）追踪身份；
   断言 展开→结构 A、收起→A′（逐边等于展开前）、再展开→A″（逐边等于 A）——不只是"内容回来了"。
5. **断言必须程序化**：每步带断言，不符即抛错退出（非 0）；打印给人看与程序断言并存，不能只打印。
6. **全展开扫查**：除单节点展开外，额外做「全部展开」，对每个可见节点断言祖先链在位——
   只测单个分支会漏掉其它分支的错位。
7. 视觉对照（截图）只做外观兜底；**结构正确性以 DOM 结构断言为准**。
